 ...
 */
$seconds_to_cache = 600; // Cache duration sent to the browser.
// Not used
...
class render3DPlayer
{
    ...
    public function __construct($user, $vr, $hr, $hrh, $vrll, $vrrl, $vrla, $vrra, $displayHair, $headOnly, $format, $ratio, $aa, $layers, $debug = false, $cache = true)
    {
        ...
        $this->vrla = $vrla;
        $this->vrra = $vrra;
        $this->head_only = ($headOnly == 'true' || $headOnly);
        $this->display_hair = ($displayHair != 'false' || $displayHair);
        $this->format = $format;
        $this->ratio = $ratio;
        $this->aa = ($aa == 'true');
        $this->layers = $layers;
    ...
    private function getPlayerSkin()
    {
        ...
        if (trim($this->playerName) == '') {
            if ($this->debug) echo 'Playername is empty';
            ...
            if ($skinURL !== false) {
                ...
                if ($this->debug) echo 'Getting skin: ' . $skinURL;
            } else {
            ...
            if ($MCa->fetchError === true) {
                ...
                if ($this->debug) echo 'An error has occured while getting the skin file';
            }
            if ($this->debug) echo 'Skin URL:' . $skinURL;
        }

        if (!$this->playerSkin) {
            // Player skin does not exist
            if ($this->debug) echo 'Falling back on default...';
            $this->playerSkin = imageCreateFromPng($this->fallback_img);
            return false;
        }
        ...
        if (imagesy($this->playerSkin) % 32 != 0) {
            // Bad ratio created
            if ($this->debug) echo 'Ratio incorrect';
            $this->fetchError = true;
            $this->playerSkin = imageCreateFromPng($this->fallback_img);
            return false;
        }
...
class img
{
    ...
    public static function createEmptyCanvas($w, $h)
    {
        ...
        if (true) {
            ...
            //$dst = imagerotate($dst, 0, $transparency, 1);
